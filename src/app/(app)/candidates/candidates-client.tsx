...
import { Search, Plus, Pencil, Trash2, Wand2, X, Phone, Briefcase } from 'lucide-react'
...
function CandidateModal({ candidate, onClose, onSave }: ModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('')
    try {
      await onSave({ name, phone, role_applied: role, stage, notes })
      onClose()
    } catch (err: unknown) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface-card rounded-lg shadow-raised w-full max-w-md z-10">
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input
                ...
                className="w-full border border-surface-borderStrong rounded px-3 py-2 text-sm text-ink focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
              />
            </div>
            <div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full border border-surface-borderStrong rounded px-3 py-2 text-sm text-ink focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary bg-surface-card"
            >
          ...
          <div>
            ...
            <textarea
              ...
              rows={3} placeholder="Any notes about this candidate…"
              className="w-full border border-surface-borderStrong rounded px-3 py-2 text-sm text-ink focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary resize-none"
            />
          </div>
...
export default function CandidatesClient({ initial }: { initial: Candidate[] }) {
  ...
  return (
    <>
      {(modal === 'add') && (
        <CandidateModal onClose={() => setModal(null)} onSave={handleAdd} />
      )}
      {(modal === 'edit' && editing) && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-surface-border bg-surface-sunken">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium text-ink-muted uppercase tracking-wide hidden lg:table-cell">Notes</th>
              ...
              <tbody className="divide-y divide-surface-border">
                {filtered.map(c => (
                  <tr key={c.id} className="hover:bg-surface-sunken/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-ink-secondary hidden lg:table-cell max-w-[200px]">
                      <p className="truncate text-xs">{c.notes || '—'}</p>
                    </td>
                    <td className="px-4 py-3">
